...
import org.eclipse.hono.command.CommandReplyMessageFilter;
...
@Scope("prototype")
@Qualifier("command")
public class CommandReplyEndpoint extends MessageForwardingEndpoint {

    public CommandReplyEndpoint(final Vertx vertx) {
        super(vertx);
    }

    @Override public String getName() {
        return CommandConstants.COMMAND_REPLY_ENDPOINT;
    }
    ...
    protected boolean passesFormalVerification(final ResourceIdentifier targetAddress, final Message message) {
        return CommandReplyMessageFilter.verify(targetAddress, message);
    }
